...
@click.command(name="delete_parser", help="[New]Delete a parser")
...
def delete_parser(
    ...
    log_type: str,
    parser_id: int) -> None:
  """Delete a parser given the Parser ID.
  ...
  delete_parser_url = url.get_dataplane_url(
      region,
      "delete_parser",
      env,
      resources)
  ...
  click.echo("Parser deleted successfully.")

  if verbose:
    api_utility.print_request_details(
        delete_parser_url, method, None, parsed_response
    )
