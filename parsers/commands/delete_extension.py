...
@click.command(name="delete_extension", help="[New]Delete an extension")
...
def delete_extension(
  ...
  """Delete a parser extension given the ParserExtension ID.
  ...
  click.echo("Deleting Parser Extension...")
  ...
  resources = {
      ...
      "log_type": log_type,
      "parser_extension": parserextension_id,
  }

  delete_extension_url = url.get_dataplane_url(
      region,
      "delete_extension",
      env,
      resources)
  ...
  response = client.request(
      method, delete_extension_url, timeout=url.HTTP_REQUEST_TIMEOUT_IN_SECS)
  parsed_response = api_utility.check_content_type(response.text)

  if response.status_code != status.STATUS_OK:
    click.echo(
        f"Error while deleting parser extension.\n"
  ...
  click.echo("Parser Extension deleted successfully.")

  if verbose:
    api_utility.print_request_details(
        delete_extension_url, method, None, parsed_response
    )
