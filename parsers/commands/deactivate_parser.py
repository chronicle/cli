...
@click.command(name="deactivate_parser", help="[New]Deactivate a parser")
...
def deactivate_parser(
    ...
    log_type: str,
    parser_id: int) -> None:
  """Deactivate a parser given the Parser ID.
  ...
  click.echo("Deactivating Parser...")
  ...
  deactivate_parser_url = url.get_dataplane_url(
      region,
      "deactivate_parser",
      env,
      resources)
  ...
  response = client.request(
      method, deactivate_parser_url, timeout=url.HTTP_REQUEST_TIMEOUT_IN_SECS)
  parsed_response = api_utility.check_content_type(response.text)

  if response.status_code != status.STATUS_OK:
    click.echo(
        f"Error while deactivating parser.\n"
  ...
  click.echo("Parser deactivated successfully.")

  if verbose:
    api_utility.print_request_details(
        deactivate_parser_url, method, None, parsed_response
    )
