...
@click.command(name="get_parser", help="[New]Get details of a parser")
@click.option(
    "-f",
    "--file-format",
...
def get_parser(
  ...
  click.echo("Fetching Parser details...")
  ...
  get_parser_url = url.get_dataplane_url(
      region,
      "get_parser",
      env,
      resources)
  ...
  if export:
    ...
    if file_format == file_utility.FILE_FORMAT_JSON:
      file_utility.export_json(export_path, parser_details_json)
    else:
  ...
  if verbose:
    api_utility.print_request_details(
        get_parser_url, method, None, parsed_response)
